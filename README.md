...
-	매니저 등록 시 로그 기록:
매니저 등록과 별개로 로그가 항상 기록되도록 @Transactional의 옵션을 활용해 독립적인 트랜잭션을 관리합니다.
...
- 누구나 접속 가능한 health check API를 구현하여 README.md에 경로를 기재합니다.
...
# Git Push 거부 트러블 슈팅: AWS Access Key 노출 문제 해결

## 문제 상황
GitHub에 코드를 푸시하는 중 다음과 같은 에러 메시지를 받았습니다:
![스크린샷 2024-10-10 오후 11 42 50](https://github.com/user-attachments/assets/db8a1974-bb21-4793-a382-80c68c234b9a)

이 에러 메시지는 AWS Access Key와 Secret Key가 `application.yml` 파일에 노출되어 GitHub의 보안 규칙에 의해 푸시가 거부된 상황을 설명합니다.

## 해결 과정

### 1. AWS 키 제거
먼저, 노출된 AWS 키를 제거해야 합니다. `application.yml` 파일에서 해당 키를 삭제하거나 환경 변수를 통해 키를 관리하도록 변경하였습니다.
...
```
### 2. .gitignore에 application.yml 추가

해당 파일이 다시 푸시되지 않도록 .gitignore 파일에 추가합니다.
